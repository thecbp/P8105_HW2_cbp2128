...
In the beginning the 2016 precipitation dataset had 12 rows, and the 2017 dataset had 12 rows. After all the data cleaning and manipulation, the resulting preciptation data is 20 rows and has more descriptive `year` and `month` descriptions. The `total` column gives us the amount of precipitation on a given `year` and `month`.
...
`tidy_precipitation` tells us that 29.93 inches of rain fell in 2017, given the data that was present for that year.
